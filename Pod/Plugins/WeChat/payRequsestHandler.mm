...
//        //设置附加数据
//        [self setAttach:ST_PAYMENT_RESERVE_DATA];
//    }
//    return self;
//}

-(instancetype)initWithConfiguration:(WeChatPayConfiguration *)configuration {
    self = [super init];
    if (self) {
        [self init:configuration.appId mch_id:configuration.mchId];
        [self setKey:configuration.privateKey];
        [self setNotifyUrl:configuration.notifyUrl];
        [self setAttach:configuration.reserveData];
    }
    return self;
}
